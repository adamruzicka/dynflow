...
require 'dynflow/executors/solid_queue/internal_job_base'
require 'dynflow/executors/solid_queue/orchestrator_jobs'
require 'dynflow/executors/solid_queue/worker_jobs'
      ...
      class Core < Abstract::Core
        ...
        def feed_pool(work_items)
          work_items.each do |new_work|
            WorkerJobs::PerformWork.set(queue: suggest_queue(new_work)).perform_later(new_work)
          end
        end
        ...
        def begin_startup!
          WorkerJobs::DrainMarker.perform_later(@world.id)
          @recovery = true
        end
