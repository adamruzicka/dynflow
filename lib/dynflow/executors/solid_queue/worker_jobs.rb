      ...
      module WorkerJobs
        class PerformWork < InternalJobBase
          def perform(work_item)
            ...
            OrchestratorJobs::HandlePersistenceError.perform_later(e, work_item)
          ensure
            step = work_item.step if work_item.is_a?(Director::StepWorkItem)
            OrchestratorJobs::WorkerDone.perform_later(work_item, step && step.delayed_events)
          end
        ...
        class DrainMarker < InternalJobBase
          def perform(world_id)
            OrchestratorJobs::StartupComplete.perform_later(world_id)
          end
        end
      end
    end
